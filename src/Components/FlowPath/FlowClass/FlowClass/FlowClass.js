...
class FlowClass extends Component{
    ...
    handleOk(flag) {
        if (flag === "ADD"){
            ...
            this.FlowClassService.installFlowClass(data).then(res=> {
                if (res.data.code > 0) {
            ...
            this.FlowClassService.updateFlowClass({categoryName:data.categoryName,categoryId:data.categoryId}).then(res=> {
                if (res.data.code > 0) {
    ...
    render() {
        ...
        return(
            <div style={{padding:'10px'}}>
                ...
                <Table size={"small"} rowSelection={rowSelection} rowKey={record => record.speciesId} loading={this.state.loading} columns={columns} dataSource={this.state.dataSource} pagination={ false }/>
                <Modal
                    title="增加项目"
...
export default Form.create("FlowClass")(FlowClass);