...
class UpdateForm extends Component{
    constructor (props) {
        ...
        this.state = {
            ...
            entityData:{},
            // 实体列表
            entityList:this.props.updateData.businessObjectEntities,
        }
    }
    getFormContent() {
        let result;
    ...
    componentWillReceiveProps(nextProps) {
        let entityList = nextProps.updateData.businessObjectEntities;
        let entityId= nextProps.updateData.businessObjectEntityRelations;
        if (entityList && entityId) {
            let mainId = entityId.filter(item=>{
                return item.type === "main"
            })
            let mainData = entityList.filter(item=>{
                return item.id === mainId[0].entityId
            })
            let childData = entityList.filter(item=>{
                return item.id !== mainId[0].entityId
            })
            this.setState({
                entityData:mainData[0],
                entityList:childData,
            })
        }
    }
    // 查询分类
        ...
        this.props.form.validateFields((err, values)=>{
            if (!err) {
                ...
                if (entityList.length === 0) {
                    ...
                    result.id = this.props.updateData.id;
                    ...
                    perItem.type = "main"
                    temp.push(perItem);
                     result.businessObjectEntityRelation = temp;
                    this.FlowProjectService.updatePro(result).then(res=>{
                        if(res.data.code> 0) {
                            this.props.onRef();
    ...
    render() {
        ...
        const checkFlag = this.props.updateData.supportDb ? true : false
        const entityName = this.state.entityData.name? this.state.entityData.name: ""
        return(
                ...
                <Row gutter={40}>
                    <Col span={4}>
                        <Form.Item label={"描述"}>
                            {getFieldDecorator("depict",{
                                initialValue:this.props.updateData.depict,
                                rules: [
                    ...
                    <Col span={5}>
                        <Form.Item label={"别名"}>
                            {getFieldDecorator("alias",{
                                initialValue:this.props.updateData.alias,
                                rules: [
                    ...
                    <Col span={5}>
                        <Form.Item label={"分类"}>
                            {getFieldDecorator("categoryId",{
                                initialValue:this.props.updateData.categoryId,
                                rules: [
                    ...
                    <Col span={5}>
                        <Form.Item label={"支持数据库"}>
                            {getFieldDecorator("supportDb")(<Checkbox checked={checkFlag}/>)}
                        </Form.Item>
                    </Col>
